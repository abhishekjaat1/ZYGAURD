...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        const nightmode = client.util.embed()
        ...
        if (option === 'enable') {
            ...
            const promises = manageableRoles.map(async (role) => {
                ...
                if (adminPermissions.length > 0) {
                    ...
                    await role
                        .setPermissions(
                            permissions,
                            `ZyGuard NIGHTMODE ENABLED`
                        )

                        .catch((err) => {
                            message.channel.send({
                            content : `Please Check My Role Position And Give Me Proper Role.`
                        })
                            return
                        })
                }
         ...
         content :       'Nightmode disabled! Restored Permissions For Manageable Roles.'
        })
        } else {
            return message.channel.send({ embeds: [nightmode] })
        }
