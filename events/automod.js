                ...
                try {
                    ...
                    if (action === 'ban') {
                        ...
                        if (message.member.bannable) {
                            try {
                                let reason = 'ZyGuard | ANTILINK | BAN'
            ...
            try {
                if (!action || action === 'mute') {
                    ...
                    if (newMessage.member.manageable) {
                        ...
                        try {
                            await newMessage.member.timeout(30 * 60 * 1000, 'ZyGuard | ANTILINK | TIMEOUT');
                ...
                if (action === 'kick') {
                    ...
                    if (newMessage.member.kickable) {
                        try {
                            await newMessage.guild.members.kick(newMessage.member.id, 'ZyGuard | ANTILINK | KICK');
                            let success = await newMessage.channel.send({
                                embeds: [
                                    client.util.embed()
                                        .setColor(client.color)
                    ...
                    if (member) {
                        try {
                            if (action === 'mute' || !action) {
                                ...
                                await member.kick('ZyGuard | ANTISPAM | KICK');
                                let successMessage = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                ...
                                await member.ban({ reason: 'ZyGuard | ANTISPAM | BAN' });
                                let successMessage = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
        ...
        if (foundBadWord) {
            if (!action || action === 'mute') {
                ...
                if (newMessage.member.manageable) {
                    if (newMessage.member.isCommunicationDisabled()) return;
                    try {
                        await newMessage.member.timeout(30 * 60 * 1000, 'ZyGuard | ANTI WORDS | TIMEOUT');
                        let success = await newMessage.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                ...
                if (newMessage.member.kickable) {
                    try {
                        await newMessage.member.kick('ZyGuard | ANTI WORDS | KICK');
                        let success = await newMessage.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                ...
                if (newMessage.member.banable) {
                    try {
                        await newMessage.member.ban('ZyGuard | ANTI WORDS | BAN');
                        let success = await newMessage.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                    ...
                    switch (action) {
                        case 'mute':
                            if (message.member.manageable && !message.member.isCommunicationDisabled()) {
                                await message.member.timeout(30 * 60 * 1000, 'ZyGuard | ANTI-NSFW | TIMEOUT');
                                let successMessage = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                        ...
                        case 'kick':
                            if (message.member.kickable) {
                                await message.guild.members.kick(message.member.id, 'ZyGuard | ANTI-NSFW | KICK');
                                let successMessage = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                        ...
                        case 'ban':
                            if (message.member.bannable) {
                                await message.guild.members.ban(message.member.id, { reason: 'ZyGuard | ANTI-NSFW | BAN' });
                                let successMessage = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
        ...
        try {
            ...
            if (urls && urls.some((url) => !isAllowedLink(url))) {
                ...
                if (!action || action === "mute") {
                    try {
                        if (message.member.manageable && !message.member.isCommunicationDisabled()) {
                            await message.member.timeout(30 * 60 * 1000, "ZyGuard | ANTIINVITE | TIMEOUT").catch((err) => { });
                            let success = await message.channel.send({
                                embeds: [
                                    client.util.embed()
                                        .setColor(client.color)
                    ...
                    try {
                        if (message.member.kickable) {
                            await message.guild.members.kick(message.member.id, "ZyGuard | ANTIINVITE | KICK").catch((err) => { });
                            let success = await message.channel.send({
                                embeds: [
                                    client.util
                                        .embed()
                    ...
                    try {
                        if (message.member.bannable) {
                            await message.guild.members.ban(message.member.id, { reason: "ZyGuard | ANTIINVITE | BAN" }).catch((err) => { });
                            let success = await message.channel.send({
                                embeds: [
                                    client.util
                                        .embed()
            ...
            if (!action || action === "mute") {
                try {
                    if (newMessage.member.manageable && !newMessage.member.isCommunicationDisabled()) {
                        await newMessage.member.timeout(30 * 60 * 1000, "ZyGuard | ANTIINVITE | TIMEOUT").catch(() => {});
                        let success = await newMessage.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                ...
                try {
                    if (newMessage.member.kickable) {
                        await newMessage.guild.members.kick(newMessage.member.id, "ZyGuard | ANTIINVITE | KICK").catch(() => {});
                        let success = await newMessage.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                ...
                try {
                    if (newMessage.member.bannable) {
                        await newMessage.guild.members.ban(newMessage.member.id, { reason: "ZyGuard | ANTIINVITE | BAN" }).catch(() => {});
                        let success = await newMessage.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
